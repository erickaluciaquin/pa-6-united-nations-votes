...
```{r}
unvotes |> 
  count(country, vote) |> # counts number of cases
  group_by(country) |> # Groups data by country
  mutate(total = sum(n)) |> #Creates new column with the total sum of votes per country
  mutate(prop_vote = n/total) |> #Creates new column with total number of votes per country out of total votes within the united nations, Proportion of Votes
  filter(country %in% c("United States", "Canada", 
                        "Germany", "France",
                        "Italy", "Japan",
                        "United Kingdom")) |> #Focuses on these countries only
  ggplot(aes(x = country, y = prop_vote, 
             fill = vote)) + # creates boxplot
  geom_col(position = position_stack()) + #It makes the height of data different values in the dataset, Variables all fall into the same stack 
  labs(x = "Group of Seven Countries",
       y = "Proportion of Votes",
       title = "Voting Record of the G7",
       fill = "Vote") + #Labels graph, fills in the Vote variable different colors
  theme_minimal() +  #Theme of Graph
  scale_fill_viridis_d(end = 0.8) + #Makes colors easier for Black and White as well as easier for people with common colorblindness
  coord_flip() # flips graph view to vertical/horizontal position
```

> Describe what the graph above demonstrates above UN voting records for the G7 
> Description of the total vote, showing proportion of how each country voted. 
...
```{r}
roll_calls |> 
  distinct(short) # removes rows with duplicate values from data frame
```

> Display of character values under "short" column of duplicated values from data frame. 
...
> Country data is missing. It is crucial because we want to know which countries voted and for what they voted in order to fully assess and understand the issues UN casts votes for. 
...
> Some of the information above is relatively useful, but other votes are not specific on their message. Thus, this can undermine the validity and purpose of the vote over time.
...
> Because there are more areas for people to vote on in the roll calls area, people might feel more pressured to vote on more items, compared to the Issues area that is more specific and less broad. Roll Calls area has more variables for people to vote on compared to issues that only has 9. 
